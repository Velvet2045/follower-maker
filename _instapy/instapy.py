...
class GuiLogger(logging.Handler):
    def emit(self, record):
        recordShort = self.format(record)
        self.edit.append(recordShort[0:120])
...
class InstaPy:
    ...
    def __init__(self,
        ...
        self.logger = self.get_instapy_logger(self.show_logs, self.txt_logger)
    ...
    def get_instapy_logger(self, show_logs, txt_logger):
        """
        ...
        if existing_logger is not None:
            ...
            if show_logs is True:
                ...
                console_handler.edit = txt_logger
    ...
    def end_sub(session):
        ...
        # delete cookies
        try:
            session.browser.delete_all_cookies()
        except Exception as exc:
            if isinstance(exc, WebDriverException):
                session.logger.exception("Error occurred while deleting cookies from web browser!\n\t{}"
        ...
        try:
            ...
            if isinstance(exc, WebDriverException):
                session.logger.exception("Error occurred while closing web browser!\n\t{}"
                                         .format(str(exc).encode("utf-8")))
        ...
        dump_follow_restriction(session.username, session.logger, session.logfolder)
        dump_record_activity(session.username, session.logger, session.logfolder)

        with open('{}followed.txt'.format(session.logfolder), 'w') as followFile:
            followFile.write(str(session.followed))

        # output live stats before leaving
        session.live_report()

        message = "Session ended!"
        highlight_print(session.username, message, "end", "info", session.logger)
        print("\n\n")
