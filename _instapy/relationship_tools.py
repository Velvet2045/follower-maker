...
def get_followers_sub(browser,
    ...
    # up querying time
    all_prior_followers = relationship_data[username][
        "all_followers"] if match is not None else None
        ...
        try:
            ...
            if not os.path.isfile(filename):
                with csv_writer_lock:
                    graphql_queries_file = open(filename, 'w', newline='')
            ...
            with csv_writer_lock:
                graphql_queries_file = open(filename, 'r')
        ...
        while has_next_data:
            ...
            if match is not None:
                ...
                if matched_followers >= match:
                    new_followers = set(all_followers) - set(
                        all_prior_followers)
                    ...
                    logger.info(
                        ...
                        "~total of {} usernames".format(
                            len(set(new_followers)), passed_time,
    ...
    if len(all_followers) > 0:
        if (store_locally is True and
            ...
            store_followers_data_sub(username, grab, all_followers, logger,
                                 logfolder)
...
def get_following_sub(browser,
    ...
    # if there has been prior graphql query, use that existing data to speed
    # up querying time
    all_prior_following = relationship_data[username][
        "all_following"] if match is not None else None
        ...
        while has_next_data:
            ...
            if match is not None:
                ...
                if matched_following >= match:
                    new_following = set(all_following) - set(
                        all_prior_following)
                    ...
                    logger.info(
                        ...
                        "~total of {} usernames".format(
                            len(set(new_following)), passed_time,
...
def get_unfollowers_sub(browser,
                    username,
                    compare_by,
                    compare_track,
                    relationship_data,
    ...
    prior_followers, selected_filename = load_followers_data_sub(username,
                                                             compare_by,
                                                             compare_track,
                                                             logger,
                                                             logfolder)
    ...
    current_followers = get_followers_sub(browser,
                                      username,
                                      "full",
                                      relationship_data,
                                      live_match,
    ...
    if len(all_unfollowers) > 0:
        current_following = get_following_sub(browser,
                                          username,
                                          "full",
                                          relationship_data,
                                          live_match,
...
def get_nonfollowers_sub(browser,
                     username,
                     relationship_data,
                     live_match,
                     store_locally,
    ...
    # get `Followers` data
    all_followers = get_followers_sub(browser,
                                  username,
                                  "full",
                                  relationship_data,
                                  live_match,
                                  ...
                                  logfolder)
    # get `Following` data
    all_following = get_following_sub(browser,
                                  username,
                                  "full",
                                  relationship_data,
                                  live_match,
    ...
    store_nonfollowers_sub(username,
                       len(all_followers),
                       len(all_following),
                       nonfollowers,
                       logger,
...
def get_mutual_following_sub(browser,
                         username,
                         relationship_data,
                         live_match,
                         store_locally,
    ...
    store_mutual_following_sub(username,
                           len(all_followers),
                           len(all_following),
                           mutual_following,
                           logger,
...
def store_followers_data_sub(username, grab, grabbed_followers, logger, logfolder):
    ...
    try:
        ...
        with csv_writer_lock:
            followers_data = open(final_file, 'w', newline='')
            json.dump(grabbed_followers, followers_data)
        logger.info(
            "Stored `Followers` data at {} local file".format(final_file))
...
def store_following_data_sub(username, grab, grabbed_following, logger, logfolder):
    """ Store grabbed `Following` data in a local storage at generated date """
    query_date = datetime.today().strftime("%d-%m-%Y")
    grabbed_following_size = len(grabbed_following)
    file_directory = "{}/relationship_data/{}/following/".format(logfolder,
                                                                 username)
    file_name = "{}{}~{}~{}".format(file_directory, query_date, grab,
                                    grabbed_following_size)
    file_index = 0
    final_file = "{}.json".format(file_name)
    ...
    try:
        ...
        with csv_writer_lock:
            following_data = open(final_file, 'w', newline='')
            json.dump(grabbed_following, following_data)
        logger.info(
            "Stored `Following` data at {} local file".format(final_file))
...
def store_all_unfollowers_sub(username, all_unfollowers, logger, logfolder):
    ...
    file_directory = "{}/relationship_data/{}" \
                     "/unfollowers/all_unfollowers/".format(
        logfolder, username)
    file_name = "{}{}~all~{}".format(file_directory, generation_date,
                                     all_unfollowers_size)
    file_index = 0
    final_file = "{}.json".format(file_name)
    ...
    try:
        ...
        with csv_writer_lock:
            unfollowers_data = open(final_file, 'w', newline='')
...
def store_active_unfollowers_sub(username, active_unfollowers, logger, logfolder):
    """ Store active Unfollowers data in a local storage at generated date """
    generation_date = datetime.today().strftime("%d-%m-%Y")
    active_unfollowers_size = len(active_unfollowers)
    file_directory = "{}/relationship_data/{}" \
                     "/unfollowers/active_unfollowers/".format(
        logfolder, username)
    file_name = "{}{}~active~{}".format(file_directory, generation_date,
                                        active_unfollowers_size)
    file_index = 0
    final_file = "{}.json".format(file_name)
    ...
    try:
        ...
        with csv_writer_lock:
            active_unfollowers_data = open(final_file, 'w', newline='')
...
def store_nonfollowers_sub(username, followers_size, following_size, nonfollowers,
                       logger, logfolder):
    """ Store Nonfollowers data in a local storage at generated date """
    generation_date = datetime.today().strftime("%d-%m-%Y")
    nonfollowers_size = len(nonfollowers)
    file_directory = "{}/relationship_data/{}/nonfollowers/".format(logfolder,
                                                                    username)
    file_name = "{}{}~[{}-{}]~{}".format(
        file_directory, generation_date, followers_size, following_size,
        nonfollowers_size)
    file_index = 0
    final_file = "{}.json".format(file_name)
    ...
    try:
        ...
        with csv_writer_lock:
            nonfollowers_data = open(final_file, 'w', newline='')
            json.dump(nonfollowers, nonfollowers_data)
        logger.info(
            "Stored Nonfollowers data at {} local file\n".format(final_file))
...
def store_fans_sub(username, followers_size, following_size, fans, logger,
               logfolder):
    """ Store Fans data in a local storage at generated date """
    generation_date = datetime.today().strftime("%d-%m-%Y")
    fans_size = len(fans)

    file_directory = "{}/relationship_data/{}/fans/".format(logfolder,
                                                            username)
    file_name = "{}{}~[{}-{}]~{}".format(
        file_directory, generation_date, followers_size, following_size,
        fans_size)
    ...
    try:
        ...
        with csv_writer_lock:
            fans_data = open(final_file, 'w', newline='')
...
def store_mutual_following_sub(username, followers_size, following_size,
    ...
    try:
        ...
        with csv_writer_lock:
            mutual_following_data = open(final_file, 'w', newline='')
...
def load_followers_data_sub(username, compare_by, compare_track, logger,
                        logfolder):
    ...
    (glob.glob("{}/{}~full*.json".format(files_location, selected_filename)))[
        0]
    with csv_writer_lock:
        followers_data_file = open(selected_file, 'r')
