...
def log_followed_pool_sub(login, followed, logger, logfolder, logtime, user_id):
    """Prints and logs the followed to
    a seperate file"""
    try:
        with csv_writer_lock:
            strRowData = []
            strRowData.append(str('{} ~ {} ~ {}'.format(logtime, followed, user_id)))
            followPool = open('{0}{1}_followedPool.csv'.format(logfolder, login), 'w', newline='')
            csv_writer = csv.writer(followPool)
            csv_writer.writerow(strRowData)

    except BaseException as e:
        logger.error("log_followed_pool_sub error {}".format(str(e)))

    # We save all followed to a pool that will never be erase
    log_record_all_followed_sub(login, followed, logger, logfolder, logtime, user_id)
...
def log_uncertain_unfollowed_pool_sub(login, person, logger, logfolder, logtime,
                                  user_id):
    ...
    try:
        with csv_writer_lock:
            strRowData = []
            strRowData.append(str('{} ~ {} ~ {},\n'.format(logtime, person, user_id)))
            followPool = open('{0}{1}_uncertain_unfollowedPool.csv'.format(logfolder, login), 'w', newline='')
            csv_writer = csv.writer(followPool)
            csv_writer.writerow(strRowData)
    except BaseException as e:
        logger.error("log_uncertain_unfollowed_pool_sub error {}".format(str(e)))
...
def log_record_all_unfollowed_sub(login, unfollowed, logger, logfolder):
    """logs all unfollowed ever to
    a seperate file"""
    try:
        with csv_writer_lock:
            followPool = open('{0}{1}_record_all_unfollowed.csv'.format(logfolder, login), 'w', newline='')
            csv_writer = csv.writer(followPool)
            csv_writer.writerow(str('{},\n'.format(unfollowed)))
    except BaseException as e:
        logger.error("log_record_all_unfollowed_pool_sub error {}".format(str(e)))
...
def log_record_all_followed_sub(login, followed, logger, logfolder, logtime,
                            user_id):
    ...
    try:
        with csv_writer_lock:
            strRowData = []
            strRowData.append(str('{} ~ {} ~ {},\n'.format(logtime, followed, user_id)))
            followPool = open('{0}{1}_record_all_followed.csv'.format(logfolder, login), 'w', newline='')
            csv_writer = csv.writer(followPool)
            csv_writer.writerow(strRowData)
    except BaseException as e:
        logger.error("log_record_all_followed_pool_sub error {}".format(str(e)))
